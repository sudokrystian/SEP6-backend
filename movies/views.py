...
def findMoviesByName(request, page_number, movie_name):
    if(request.method == 'GET'):
        parameters = {"api_key": API_KEY,
...
def findPeopleByName(request, page_number, person_name):
    if(request.method == 'GET'):
        parameters = {"api_key": API_KEY,
...
def movieLists(request):
    if request.user.is_authenticated:
        if(request.method == 'GET'):
            lists_by_user = MovieList.objects.filter(
...
def becomeUser(request):
    if(request.method == 'POST'):
        ...
        try:
            ...
            user = authenticate(username=username_from_json,
